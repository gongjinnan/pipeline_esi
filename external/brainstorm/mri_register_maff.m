function Transf = mri_register_maff(sMri)
% MRI_REGISTER_MAFF:  Register a volume on the MNI ICM152 template, using SPM's mutual information algorithm.
...
% Get template file
tpmFile = bst_fullfile(cd, 'defaults', 'spm', 'TPM.nii');
...
% === ORIGINAL VERSION: LOAD WITH SHEAR ???!!!! ===
%     % Load volume
%     V  = spm_vol([
%     [tpmFile ',1']
%     [tpmFile ',2']
...
%         s         = zeros(V(1).dim(1:2));
%         for iv = 1:Nv
%             tmp                = spm_slice_vol(V(iv),M,V(1).dim(1:2),0);
%             tpm.dat{iv}(:,:,i) = max(min(tmp,1),0);
%             s                  = s + tmp;
%         end
%         t = s>1; 
%         if any(t)
